...
> **📐 公司分类索引：按类型 + 按技术栈** (v26)
> **实测公司**：44+ 家（v3-v26 累计）
> **实测结果**:21 家 URL/API/CSR 定向搜索有效 + 1 家 🏠 | **📐 新增：按公司类型分类 + 按技术栈分类**
...
| 公司 | 章节 | 系统 | 算法方向 |
|---|---|---|---|
| **蔚来** | [10a](#10a-蔚来---✅-url搜索有效-v12-飞书系) | 飞书 `?keyword=X` | AD感知/端到端/VLA/SLAM |
| **小鹏** | [10b](#10b-小鹏---✅-csr-url搜索-v24-重测) | 飞书 `?keywords=X&keyword=X` | AD RL+VLA / 机器人具身智能 (116岗) |
| **理想** | [10c](#10c-理想汽车--✅-get-api-v26-重测) | REST API `job_function_ids=1/8/92` | AD VLA/感知 / 基座模型RL / 具身智能 / AI音乐 |

### 🏢 互联网大厂 (9)

| 公司 | 章节 | 系统 | 算法方向 |
|---|---|---|---|
| **字节** | [1](#1-字节跳动-bytedance---✅-url搜索有效-playwright-实测) | `?keywords=X&project=...` | 推荐/搜索/NLP/CV/多模态/LLM/广告/语音/风控/OR/AI Infra/机器人 |
| **美团** | [2](#2-美团---✅-url搜索有效) | `?hiringType=4_1&keyword=X` | 推荐/搜索/广告/NLP/CV/LLM/OR/风控 |
| **腾讯** | [3](#3-腾讯---✅-url搜索有效) | `?keyword=X` | 推荐/搜索/NLP/CV/LLM/游戏AI |
| **阿里** | [4](#4-阿里巴巴---❌-url-搜索失效-playwright-实测) | ❌ URL 失效 | — |
| **百度** | [5](#5-百度---✅-项目码篏-v20-重测) | SSR `?projectType=1/3/4` | AIDU(11顶级AI) / LLM / 搜索 / 自动驾驶 |
| **京东** | [6](#6-京东---✅-post-api--人才专项-v21-重测) | POST API | TGT(127顶级AI) / 搜推 / LLM / AI Infra |
| **快手** | [15](#15-快手校招---✅-项目篏--类目码-v18-重测) | API `recruitSubProjectCodes` + 类目码 | 大模型Infra / 推荐 / 多模态CV / 搜索 / 广告 / NLP / AIGC |
| **拼多多** | [8](#8-拼多多---🏠-主页站内筛-v17-新域名) | 🏠 careers.pddglobalhr.com | 搜索/推荐 |
| **滴滴** | [9](#9-滴滴---✅-moka-系统-v12) | Moka `#/jobs?project=2027` | 调度/定价/ETA/地图/SLAM |

### 🛒 电商/社区 (4)

| 公司 | 章节 | 系统 | 算法方向 |
|---|---|---|---|
| **小红书** | [10d](#10d-小红书---✅-post-api-v23-重测) | POST API | REDstar 16岗 / Ace 38岗 搜推/多模态/LLM |
| **知乎** | [12](#12-知乎---✅-url搜索有效-moka-系统) | Moka `#/jobs?keyword=X` | NLP/搜索/推荐/LLM |
| **得物** | [10f](#10f-得物--✅-csr-url搜索-v25-重测) | 飞书 `?keywords=X` | 研究型实习生 LLM推荐/VLM搜索 |
| **Keep** | [19](#19-keep---✅-招聘主页完整-v14) | 校招主页 | 运动科技AI |

### 📱 硬件/智能设备 (5)

| 公司 | 章节 | 系统 | 算法方向 |
|---|---|---|---|
| **华为** | [7](#7-华为---✅-get-api-v22-重测) | GET API `jobType=2` | 198校招(含36 AI) / 大模型 / AI Infra / 自动驾驶 |
| **小米** | [11](#11-小米---✅-url搜索有效-v16-飞书系统) | 飞书 `?keywords=X` | 软件研发类含算法 |
| **OPPO** | [10g](#10g-oppo---✅-校招主页--调度-api-v13) | API `careers.oppo.com/openapi/...` | 博士AI / 语音 / 推荐 |
| **vivo** | (header表) | 前端JS搜索 | — |
| **大疆** | [10e](#10e-大疆---✅-moka-系统--关键词哈希篏-v19-重测) | Moka `#/jobs?keyword=X` | SLAM / 感知 / 规划控制 / 飞控 (54岗) |

### 🤖 AI 独角兽 + CV 四小龙 (8)

| 公司 | 章节 | 系统 | 算法方向 |
|---|---|---|---|
| **智谱/月之暗面/MiniMax/百川** | [13](#13-ai-独角兽---moka--飞书-系统统一格式) | Moka/飞书 | LLM全栈 |
...
| 公司 | 章节 | 系统 | 算法方向 |
|---|---|---|---|
| **360** | [17](#17-360-集团---✅-校招主页完整-v14) | 校招主页 | AI安全 |
| **用友** | [25](#25-用友) | 校招主页 | 企业AI |
| **深信服** | [31](#31-深信服) | 🏠 | 网络安全AI |
| **摩尔线程** | [32](#32-摩尔线程) | 🏠 | GPU/AI芯片 |

| 🏨 差旅/教育/直播/医疗 (4)

| 公司 | 章节 | 系统 | 算法方向 |
|---|---|---|---|
| **携程** | [16](#16-携程校招---✅-完整校招系统-v14) | 校招主页 | 搜索/推荐/定价 |
| **猿辅导** | [26](#26-猿辅导) | 校招主页 | 教育AI |
| **虎牙** | [24](#24-虎牙) | 校招主页 | 直播AI |
| **阿里健康** | [33](#33-阿里健康) | 🏠 | 医疗AI |

### 🔌 人事系统/生态 (2)

| 公司 | 章节 | 系统 |
|---|---|---|
| **美团系** (大众点评/猫眼/酒旅) | [20](#20-美团系) | 跟美团共用系统 |
| **飞书** (字节系) | [21](#21-飞书) | 字节系共用系统 |
| **网易+有道** | [23](#23-网易--网易有道) | 校招主页 |
...
| 公司 | 具体方向 |
|---|---|
| 字节 | 预训练/SFT/RLHF/推理加速 (Top Seed) |
| 美团 | LongCat/Agent |
| 腾讯 | LLM |
| 百度 | AIDU → LLM方向 (11顶级) |
| 京东 | TGT → LLM方向 (127顶级) |
| 华为 | 大模型算法 + AI Infra |
| 快手 | 基础大模型推理引擎 (J1013 14岗) / Agent (J1001 8岗) |
| 小红书 | REDstar → LLM + Ace → LLM方向 |
| 理想 | **基座模型RL研究员-物理智能体** ✨ |
| 得物 | LLM 生成式重排/召回/搜索 |
| 智谱/MiniMax/月之暗面/百川 | LLM 全栈 (预训练/对齐/推理) |
| 知乎 | LLM 搜索/NLP |
...
| 公司 | 具体方向 |
|---|---|
| 字节 | 推荐(CTR/CVR)/搜索(RAG/向量)/广告(Auction) |
| 美团 | 外卖推荐/搜索/广告 |
| 腾讯 | 推荐/搜索 |
| 百度 | 搜索 (核心业务) |
| 京东 | 推荐/搜索 (TGT专项) |
| 拼多多 | 搜索/推荐 |
| 快手 | 推荐(J1005 10岗) / 搜索(J1004 4岗) / 广告(J1006 4岗) |
| 小红书 | REDstar搜推 / Ace搜推 |
| 知乎 | 搜索/推荐/NLP |
| 得物 | LLM推荐(生成式召回/排序)/VLM搜索 |
| 携程 | 搜索/推荐/定价 |
...
| 公司 | 具体方向 |
|---|---|
| 字节 | CV(检测/分割/AIGC)/多模态(视频理解/跨模态) |
| 美团 | 配送视觉/菜品识别 |
| 腾讯 | CV/多模态 |
| 快手 | 多模态大模型Keye/视频生成(J1007 12岗) |
| 小红书 | REDstar多模态 / Ace多模态 |
| 商汤/旷视/依图/云从 | CV全栈 |
| 大疆 | 感知/SLAM |
| 理想 | 视觉感知/多模态算法 |
| 蔚来 | AD感知 |
...
|---|---|
| 字节 | 训练框架/推理引擎/MLSys |
| 快手 | 大模型Infra(J1013 14岗)/推理引擎/调度/GPU |
| 京东 | AI Infra (TGT专项) |
| 理想 | 大模型训练推理加速 / AI系统开发 / AI云原生 |
| 华为 | 推理引擎 / AI芯片 (昇腾) |
| 百度 | AI Infra / 昆仑芯片 |
...
| 字节 | ASR/TTS/语音合成(豆包) |
| OPPO | 语音算法(博士) |
| 快手 | 音频/语音/音乐AIGC(J1010 1岗) / 智能编码(J1011 4岗) |
| 理想 | 语音/全模态算法 / AI音乐生成 |
...
|---|---|---|
| 教育AI | 猿辅导 | 在线教育 |
| 医疗AI | 22家 → [data/03-medical-companies.md](data/03-medical-companies.md) | 影像AI/AI制药/医疗NLP/互联网医疗/医院信息化 |
| 运动AI | Keep | 运动科技 |
| 芯片 | 华为/摩尔线程/OPPO | AI芯片/NPU/GPU |
| 安全 | 360/深信服 | 网络安全 |
| 直播 | 虎牙 | 直播AI |
| 企业服务 | 用友 | 企业AI |

> 💡 **使用方式**：按公司类型找 → 看公司竞争格局。按技术栈找 → 看岗位竞争格局（搜推/LLM 最拥挤，具身智能/自动驾驶/语音相对稀疏）。
